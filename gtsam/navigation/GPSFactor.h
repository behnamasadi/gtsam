...
class GTSAM_EXPORT GPSFactorLever: public NoiseModelFactorN<Pose3> {
...
private:
  ...
  Point3 nT_; ///< Position measurement in cartesian coordinates
  Point3 lever_; ///< Position measurement in cartesian coordinates
...
public:
  ...
  /// shorthand for a smart pointer to a factor
  typedef std::shared_ptr<GPSFactorLever> shared_ptr;

  /// Typedef to this class
  typedef GPSFactorLever This;

  /** default constructor - only use for serialization */
  GPSFactorLever(): nT_(0, 0, 0),lever_(0,0,0) {}

  ~GPSFactorLever() override {}
   ...
   * @param model Gaussian noise model
   */
  GPSFactorLever(Key key, const Point3& gpsIn, const Point3& leverIn, const SharedNoiseModel& model) :
      Base(model, key), nT_(gpsIn), lever_(leverIn) {
  }
